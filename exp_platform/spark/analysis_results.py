...
def _normalize_df(df: DataFrame) -> DataFrame:
    ...
    # enforce types (timestamps: accept timestamp, string, or python datetime -> normalize with to_timestamp)
    out = (
        df
    ...
    # timestamps: if already timestamp keep; else try parse; else null
    out = out.withColumn("computed_ts", F.to_timestamp(F.col("computed_ts")))
    out = out.withColumn("analysis_ts", F.to_timestamp(F.col("analysis_ts")))
    return out
...
def upsert_analysis_results(
    ...
    *,
    ...
    Deletes only the rows for this experiment (and optional metric subset), then appends normalized rows.
    """
    ...
    df.select(*ANALYSIS_RESULTS_COLS).writeTo(full_table).append()
